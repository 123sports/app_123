...
const searchSchema = z.object({
  ...
  ym: fallback(z.string().regex(/^\d{4}-\d{2}$/), format(new Date(), "yyyy-MM")).default(
...
function AdminFinanceiro() {
  ...
  const load = useCallback(async () => {
    ...
    const [{ data: pr }, { data: cs }, { data: bs }, { data: ops }] = await Promise.all([
      ...
      supabase
        .from("bookings")
        .select(
          "id, user_id, booking_date, start_hour, type, status, payment_status, payment_method, card_operator_id, amount_cents",
        )
  ...
  const savePrice = async (id: string, price_cents: number) => {
    ...
    if (error)
      return toast.error(error?.message ?? "Não foi possível atualizar o preço. Tente de novo.");
    toast.success("Preço atualizado");
  };

  const addCost = async () => {
    if (!newCost.description || !newCost.amount)
      return toast.error("Descrição e valor obrigatórios");
    ...
    if (error)
      return toast.error(error?.message ?? "Não foi possível adicionar o custo. Tente de novo.");
  ...
  const delCost = async (id: string) => {
    ...
    if (error)
      return toast.error(error?.message ?? "Não foi possível excluir o custo. Tente de novo.");
  ...
  const cash = paid
    .filter((b) => b.payment_method === "dinheiro")
    .reduce((s, b) => s + (b.amount_cents ?? 0), 0);
  const card = paid
    .filter((b) => b.payment_method === "cartao")
    .reduce((s, b) => s + (b.amount_cents ?? 0), 0);
  const pix = paid
  ...
  const fixedMonth = costs
    .filter((c) => c.recurrence === "mensal")
    .reduce((s, c) => s + (c.amount_cents ?? 0), 0);
  const oneOffMonth = costs
    .filter((c) => c.recurrence === "avulso" && c.incurred_on?.slice(0, 7) === ymStr)
    .reduce((s, c) => s + (c.amount_cents ?? 0), 0);
  ...
  if (revenue === 0 && totalCost === 0)
    health = {
      ...
      tip: "Cadastre custos e receitas para acompanhar.",
    };
  else if (result < 0)
    health = {
      label: "Atenção",
      color: "text-destructive",
      tip: "Operação no vermelho neste mês.",
    };
  else if (margin < 15)
    health = {
      label: "Apertado",
      color: "text-yellow-500",
      tip: "Margem baixa. Avalie preços ou volume.",
    };
  else if (margin < 35)
    health = { label: "Saudável", color: "text-primary", tip: "Operação equilibrada." };
  ...
  const filteredPaid = useMemo(
    () =>
      bookings.filter((b) => {
        ...
        if (fStatus === "all" && !(b.payment_status === "pago" || b.payment_status === "pendente"))
          return false;
  ...
  const goTab = (t: "visao" | "receitas" | "custos") =>
    navigate({ search: (p: any) => ({ ...p, tab: t }) });
  const shiftMonth = (delta: number) =>
    navigate({
      search: (p: any) => ({ ...p, ym: format(addMonths(monthDate, delta), "yyyy-MM") }),
    });
        ...
        actions={
          <div className="flex items-center gap-2 rounded-full border border-border bg-card px-2 py-1.5">
            <button
              ...
              className="btn-bounce rounded-full p-1 hover:bg-accent"
            >
        ...
        <div className="stack-app">
          <section className="grid auto-rows-fr gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <Kpi
              ...
              label="Receita"
              value={brl(revenue)}
              sub={`${paid.length} reservas pagas`}
            />
            <Kpi
              icon={TrendingDown}
              label="Custos"
              value={brl(totalCost)}
              accent="bad"
              sub={`${brl(fixedMonth)} fixos · ${brl(oneOffMonth)} avulsos`}
            />
            <Kpi
              icon={TrendingUp}
              label="Resultado"
              value={brl(result)}
              accent={result >= 0 ? "good" : "bad"}
              sub={`Margem ${margin}%`}
            />
            <Kpi
              icon={Activity}
              label="Saúde"
              value={<span className={health.color}>{health.label}</span>}
              sub={health.tip}
            />
          </section>
          ...
          <section className="grid auto-rows-fr gap-4 md:grid-cols-2">
            <div className="plane h-full">
              ...
              <div className="mb-2 flex items-baseline justify-between text-sm">
                ...
                <span className="font-semibold">
                  {brl(revenue)} / {brl(breakeven)}
                </span>
              </div>
              <div className="h-3 overflow-hidden rounded-full bg-muted">
                <div
                  className={`h-full transition-all ${revenue >= breakeven ? "bg-primary" : "bg-yellow-500"}`}
            ...
            <div className="plane h-full">
              ...
              <div className="mt-3 flex items-center justify-between type-small text-muted-foreground">
                ...
                <span className="type-data font-semibold text-foreground">{brl(ticket)}</span>
              </div>
              <div className="flex items-center justify-between type-small text-muted-foreground">
                <span>A receber (pendente)</span>
                <span className="type-data font-semibold text-foreground">{brl(aReceber)}</span>
              </div>
            </div>
          </section>
        ...
        <div className="stack-app">
          ...
          <div className="plane plane-compact flex flex-wrap items-center gap-2 text-sm">
            ...
            <input
              type="date"
              value={fDay}
              min={format(startOfMonth(monthDate), "yyyy-MM-dd")}
              max={format(endOfMonth(monthDate), "yyyy-MM-dd")}
            ...
            <table className="w-full text-sm">
                  ...
                  <tr key={b.id} className="border-b border-border/60 hover:bg-secondary">
                    <td className="p-3 whitespace-nowrap">
                      <div className="type-data font-medium">
                        {format(new Date(b.booking_date + "T00:00:00"), "dd/MM")}
                      </div>
                    ...
                    <td className="p-3 text-xs">
                      {b.payment_method ? PAY_LABEL[b.payment_method] : "—"}
                    </td>
                    <td className="p-3 text-xs text-muted-foreground">
                      {b.card_operator_id ? (operators[b.card_operator_id] ?? "—") : "—"}
                    </td>
                    <td className="p-3">
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs ${b.payment_status === "pago" ? "bg-primary/15 text-primary" : "bg-yellow-500/15 text-yellow-600"}`}
                      >
                        {b.payment_status}
                      </span>
                    </td>
                    <td className="p-3 text-right type-data font-semibold">
                      {brl(b.amount_cents ?? 0)}
                    </td>
                  </tr>
                ))}
              ...
              {filteredPaid.length > 0 && (
                <tfoot className="border-t-2 border-border bg-secondary">
                  <tr>
                    <td colSpan={6} className="p-3 text-right type-eyebrow text-muted-foreground">
                      Total
                    </td>
                    <td className="p-3 text-right type-data font-bold">
                      {brl(filteredPaid.reduce((s, b) => s + (b.amount_cents ?? 0), 0))}
                    </td>
                  </tr>
                </tfoot>
              )}
          ...
          {/* Tabela de preços */}
          <section className="plane">
            <h2 className="mb-4 type-h3">Produtos e valores por aluno</h2>
            <div className="grid auto-rows-fr gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {pricing.map((p) => (
                <div key={p.id} className="rounded-xl bg-secondary p-3">
                  <div className="type-small text-muted-foreground">
                    {p.display_name ?? TYPE_LABEL[p.booking_type] ?? p.booking_type}
                  </div>
                  {p.student_capacity && (
                    <div className="type-micro text-muted-foreground">
                      {p.student_capacity === 1
                        ? "1 aluno por horário"
                        : `Até ${p.student_capacity} alunos por horário`}
                    </div>
                  )}
              ...
              <table className="w-full text-sm">
                <thead className="border-b border-border text-left type-eyebrow text-muted-foreground">
                  <tr>
                    ...
                    <th className="p-2">Recorrência</th>
                ...
                <tbody>
                  {costs.map((c) => (
                    <tr key={c.id} className="border-b border-border/60">
                      ...
                      <td className="p-2 type-data">
                        {format(new Date(c.incurred_on + "T00:00:00"), "dd/MM/yy")}
                      </td>
                      <td className="p-2 text-right type-data font-semibold">
                        {brl(c.amount_cents)}
                      </td>
                      <td className="p-2 text-right">
                        <button
                          onClick={() => delCost(c.id)}
                          className="btn-bounce text-destructive hover:opacity-70"
                        >
...
function Kpi({
  ...
  return (
    <div className="plane h-full">
      ...
      <div className={`mt-2 type-data font-bold ${small ? "text-xl" : "text-2xl"} ${c}`}>
        {value}
      </div>
