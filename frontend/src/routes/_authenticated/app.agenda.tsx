...
function Agenda() {
  ...
  const loadMonth = useCallback(async () => {
    ...
    const [{ data: bs }, { data: bls }] = await Promise.all([
      (supabase as any).from("bookings_occupancy")
        .select("id, user_id, professor_id, booking_date, start_hour, type, status, payment_status, checkout_order_id, hold_expires_at")
  ...
  const cancel = async (id: string) => {
    ...
    if (b) {
      ...
      if (b.checkout_order_id && b.payment_status === "pago") {
        toast.error("Reserva paga requer cancelamento pelo professor/admin para tratar eventual estorno.");
        return;
      }
    }
        ...
        <div className="space-y-4 plane">
            ...
            >
              {TYPES.map((t) => (
                <option key={t.v} value={t.v} disabled={pricing[t.v] == null}>
                  {t.label}{pricing[t.v] == null ? " · indisponível" : ` · ${brl(pricing[t.v])}/h`}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <div className="flex items-center justify-between border border-input bg-background px-3 py-2 text-sm">
              ...
              <span className="text-xs text-muted-foreground">Confirmação automática</span>
            </div>
          </div>
              ...
              <div className="mt-4 space-y-2 rounded-xl border border-primary/30 bg-primary/5 p-3">
                ...
                <div className="flex items-center justify-between border-y border-primary/20 py-2">
                  ...
                  <strong className="type-data text-lg">
                    {pricing[type] == null ? "Preço indisponível" : brl(pricing[type] * pendingHours.size)}
                  </strong>
                </div>
                {pricing[type] != null && (
                  <div className="text-right text-xs text-muted-foreground">
                    {brl(pricing[type])} × {pendingHours.size} horário{pendingHours.size > 1 ? "s" : ""}
                  </div>
                )}
                <div className="flex gap-2">
                  <button
                    onClick={confirmBooking}
                    disabled={loading || pricing[type] == null}
          ...
          {selectedBookings.some((b) => b.user_id === userId) && (
            <div className="border-t border-border pt-4">
              ...
              <ul className="space-y-1 text-sm">
                {selectedBookings.filter((b) => b.id && b.user_id === userId).map((b) => (
                  <li key={b.id} className="flex justify-between">
                    <span className="type-data">
                      {String(b.start_hour).padStart(2, "0")}:00 · {labelType(b.type)}
                      {b.payment_status === "pendente" ? " · aguardando Pix" : ""}
                    </span>
                    <button onClick={() => b.id && cancel(b.id)} className="text-xs text-destructive hover:underline">
                      {b.payment_status === "pendente" ? "cancelar cobrança" : "cancelar"}
                    </button>
                  </li>
                ))}
              </ul>
