...
function AdminOperadoras() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Operadoras de cartão</h1>
        <p className="text-muted-foreground">Cadastre as operadoras e suas taxas para cálculo correto.</p>
      </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
        <div className="mb-4 grid gap-2 sm:grid-cols-[1fr_140px_auto]">
          ...
          <button onClick={add} className="btn-bounce inline-flex items-center gap-1 rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground">
        ...
        <ul className="divide-y divide-border">
          {list.map((o) => (
            <li key={o.id} className="flex items-center gap-3 py-3">
              <div className="flex-1">
                ...
                <div className="text-xs text-muted-foreground">Taxa: {o.fee_percent}%</div>
              </div>
              <input
              ...
              <button
                ...
                className={`rounded-full px-3 py-1 text-xs font-semibold ${o.active ? "bg-primary/20 text-primary" : "bg-muted text-muted-foreground"}`}
              >
          ...
          {list.length === 0 && <p className="py-6 text-center text-sm text-muted-foreground">Nenhuma operadora.</p>}
        </ul>
      </div>
    </div>
  );
