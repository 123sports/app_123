...
import { Loader2, ImageIcon, Store, MessageCircle, X, ShoppingCart, Plus, Minus, Trash2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold"><Store className="h-6 w-6 text-primary" /> Loja</h1>
          <p className="text-sm text-muted-foreground">Equipamentos e acessórios anunciados pelo seu professor.</p>
        </div>
        <button
          onClick={() => { playPop(); setCartOpen(true); }}
          className="btn-bounce relative inline-flex items-center gap-2 rounded-full border border-border bg-card px-4 py-2 text-sm font-semibold shadow-soft hover:border-primary"
        >
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((it) => {
            ...
            return (
              <article key={it.id} className="overflow-hidden rounded-2xl border border-border bg-card text-left shadow-soft transition hover:border-primary">
                <button type="button" onClick={() => { playPop(); setOpen(it); }} className="btn-bounce w-full text-left">
                  ...
                  <div className="space-y-1 p-4">
                    ...
                    <p className="pt-1 text-lg font-bold text-primary">{(it.price_cents / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p>
                    {it.track_stock && (
                      <p className={`text-xs ${soldOut ? "text-destructive font-medium" : "text-muted-foreground"}`}>
...
function ItemModal({ item, img, defaultWhats, onClose, onAddToCart }: { item: Item; img?: string; defaultWhats: string; onClose: () => void; onAddToCart: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-background/80 backdrop-blur sm:items-center" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-3xl border border-border bg-card shadow-soft sm:rounded-3xl">
        ...
        <div className="space-y-3 p-6">
          <div className="flex items-start justify-between gap-3">
            ...
            <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${item.condition === "novo" ? "bg-primary/15 text-primary" : "bg-secondary text-foreground"}`}>
              {item.condition === "novo" ? "Novo" : "Usado"}
            </span>
          </div>
          {item.category && <p className="text-xs text-muted-foreground">{item.category}</p>}
          <p className="text-2xl font-bold text-primary">{(item.price_cents / 100).toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p>
          {item.track_stock && (
            <p className={`text-xs ${soldOut ? "text-destructive font-medium" : "text-muted-foreground"}`}>
          ...
          <button
            onClick={onAddToCart}
            disabled={soldOut}
            className="btn-bounce inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground shadow-glow disabled:opacity-50"
          >
...
function CartDrawer({ cart, itemMap, imgs, defaultWhats, onClose, onQty, onRemove, onClear }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-background/80 backdrop-blur" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-border bg-card shadow-soft">
        <div className="flex items-center justify-between border-b border-border p-5">
          <h2 className="flex items-center gap-2 text-lg font-semibold"><ShoppingCart className="h-5 w-5 text-primary" /> Seu carrinho</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {lines.length === 0 ? (
            ...
            <ul className="space-y-3">
              {lines.map(({ line, item }) => (
                <li key={item.id} className="flex gap-3 rounded-2xl border border-border bg-background p-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      <button onClick={() => onQty(item.id, -1)} className="rounded-full border border-border p-1 hover:bg-secondary"><Minus className="h-3 w-3" /></button>
                      <span className="min-w-[24px] text-center text-sm font-semibold">{line.qty}</span>
                      <button onClick={() => onQty(item.id, +1)} className="rounded-full border border-border p-1 hover:bg-secondary"><Plus className="h-3 w-3" /></button>
        ...
        {lines.length > 0 && (
          <div className="space-y-3 border-t border-border p-5">
            ...
            <button
              onClick={checkout}
              className="btn-bounce inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#25D366] px-5 py-3 text-sm font-semibold text-white shadow-glow"
            >
              <MessageCircle className="h-4 w-4" /> Finalizar no WhatsApp
            </button>
            <button onClick={onClear} className="w-full text-center text-xs text-muted-foreground hover:text-destructive">
              Esvaziar carrinho
            </button>
          </div>
        )}
